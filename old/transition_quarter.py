...
# -------------------------
# Tunable board-game parameters
# -------------------------
# Deterministic "expected" long gain (LG). Clamped by distance to goal in apply_leaf.
LG_MEAN_YARDS = 50
...
def kickoff_state(receiving_poss: int, t: int) -> State:
    x = 20 if receiving_poss == US else 80
    return State(poss=receiving_poss, x=x, down=1, to_first=10, t=t)


def new_possession_state(new_poss: int, x_phys: int, t: int) -> State:
    x_phys = clamp_x(x_phys)
    return State(poss=new_poss, x=x_phys, down=1, to_first=first_down_to_first(new_poss, x_phys), t=t)
...
def punt_transition(s: State) -> Transition:
    ...
    if s.poss == US and y2 >= 100:
        return Transition(kickoff_state(receiving_poss=THEM, t=t2), 0.0, None)
    if s.poss == THEM and y2 <= 0:
        return Transition(kickoff_state(receiving_poss=US, t=t2), 0.0, None)

    y2 = clamp_x(y2)
    return Transition(new_possession_state(1 - s.poss, y2, t2), 0.0, None)
...
def apply_leaf(s: State, leaf: Leaf, *, consume_time: bool) -> Transition:
    ...
    def score_check(off_poss: int, x2: int) -> Optional[Transition]:
        ...
        if off_poss == US and x2 >= 100:
            return Transition(kickoff_state(receiving_poss=THEM, t=t2), +7.0, "TD")
        if off_poss == THEM and x2 <= 0:
            return Transition(kickoff_state(receiving_poss=US, t=t2), -7.0, "TD")

        # Safety
        if off_poss == US and x2 <= 0:
            return Transition(kickoff_state(receiving_poss=THEM, t=t2), -2.0, "SAFETY")
        if off_poss == THEM and x2 >= 100:
            return Transition(kickoff_state(receiving_poss=US, t=t2), +2.0, "SAFETY")
        return None

    if leaf.kind == "first":
        return Transition(
            State(s.poss, s.x, 1, first_down_to_first(s.poss, s.x), t2),
            0.0,
            None
        )
    ...
    if leaf.kind == "incomplete":
        ...
        down2 = s.down + 1
        if down2 >= 5:
            return Transition(new_possession_state(1 - s.poss, x2, t2), 0.0, None)
        return Transition(State(s.poss, x2, down2, s.to_first, t2), 0.0, None)
    ...
    if leaf.kind == "penalty":
        ...
        to2 = s.to_first - y
        if to2 <= 0:
            return Transition(State(s.poss, x2, 1, first_down_to_first(s.poss, x2), t2), 0.0, None)
        return Transition(State(s.poss, x2, s.down, to2, t2), 0.0, None)
    ...
    if leaf.kind == "lod":
        ...
        if down2 >= 5:
            return Transition(new_possession_state(1 - s.poss, x2, t2), 0.0, None)
        return Transition(State(s.poss, x2, down2, to2, t2), 0.0, None)

    if leaf.kind == "yards":
        y = leaf.yards

        # LG sentinel -> deterministic "expected" long gain, clamped by distance to goal
        if y >= 10**8:
            y = min(LG_MEAN_YARDS, dist_to_goal(s.poss, s.x))
    ...
    if leaf.kind == "fumble":
        return Transition(new_possession_state(1 - s.poss, s.x, t2), 0.0, None)
    ...
    if leaf.kind == "int":
        ...
        x2 = clamp_x(x2)
        return Transition(new_possession_state(new_poss, x2, t2), 0.0, None)
...
def apply_chain(s: State, chain: List[Leaf]) -> Transition:
    ...
    for lf in chain[1:]:
        ...
        reward += float(tr2.reward)
        if tr2.next_state is None or tr2.terminal is not None:
            return Transition(tr2.next_state, reward, tr2.terminal)
        s2 = tr2.next_state

    return Transition(s2, reward, None)
