...
Let $k'= \textsc{TimeoutReduce}(k)$ and define the four continuation values from the
resulting state $s'$ (looked up from the appropriate timeout-count slice):
\begin{align*}
  a &= V^{(\theta_1-1,\,\theta_2-1)}(s',\,k') \quad \text{(both call)},\\
  b &= V^{(\theta_1-1,\,\theta_2)}(s',\,k')   \quad \text{(offense calls only)},\\
  c &= V^{(\theta_1,\,\theta_2-1)}(s',\,k')   \quad \text{(defense calls only)},\\
  d &= V^{(\theta_1,\,\theta_2)}(s',\,k)       \quad \text{(neither calls)}.
\end{align*}

The offense announces its decision first; the defense then responds optimally.  If the
offense calls, the clock is stopped and the defense can call as well---but since the
clock has already stopped, calling only costs the defense a timeout without further
benefit, so the defense calls iff $a < b$ (i.e., spending its timeout lowers the
offense's continuation value).  The offense therefore receives $\min(a,b)$ when it calls.
If the offense does not call, the defense calls iff $c < d$, giving the offense $\min(c,d)$.
The offense maximizes:
\begin{equation}
  \label{eq:to_subgame}
  V_{\text{TO}} = \max\!\bigl(\min(a,b),\; \min(c,d)\bigr).
\end{equation}
...
$V^{(\theta_1-1,\theta_2)}$, $V^{(\theta_1,\theta_2-1)}$, and
$V^{(\theta_1-1,\theta_2-1)}$ as needed.  Tables that are no longer needed as predecessors
are evicted from memory; at most four tables reside in RAM simultaneously.
