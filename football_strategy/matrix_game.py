...
def _lookup_with_to(
    ...
    # d: neither calls — normal ticks, same TOs
    d = lookup_val(to_store.get(to_off, to_def), tmpl, q, tau)
    ...
    if effective_off:
        try:
            b = lookup_val(to_store.get(to_off - 1, to_def), tmpl_r, q, tau)
        except KeyError:
            effective_off = False
    ...
    if effective_def:
        try:
            c = lookup_val(to_store.get(to_off, to_def - 1), tmpl_r, q, tau)
        except KeyError:
            effective_def = False
